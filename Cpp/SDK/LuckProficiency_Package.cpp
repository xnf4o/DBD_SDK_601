...
namespace CG
{
	...
	float ULuckProficiency_C::GetValue(class UChargeableInteractionDefinition* chargeableInteraction, class ADBDPlayer* Player)
	{
		...
		if (!fn)
			fn = UObject::FindObject<UFunction>("Function LuckProficiency.LuckProficiency_C.GetValue");
	 ...
	 * Function:
	 * 		RVA    -> 0x00000000
	 * 		Name   -> PredefindFunction ULuckProficiency_C.StaticClass
	...
	UClass* ULuckProficiency_C::StaticClass()
	{
		...
		if (!ptr)
			ptr = UObject::FindClass("BlueprintGeneratedClass LuckProficiency.LuckProficiency_C");
		return ptr;
	}
